...
class TestLoadErrors(unittest.TestCase):
    def test_path_does_not_exist(self):
        with self.assertRaisesRegex(
            MetadataFileError, "Metadata file path doesn't exist"
        ):
            Metadata.load("/qiime2/unit/tests/hopefully/this/path/does/not/exist")

    def test_path_is_directory(self):
        fp = get_data_path("valid")

        with self.assertRaisesRegex(
            MetadataFileError, "path points to something other than a " "file"
        ):
            Metadata.load(fp)
    ...
    def test_non_utf_8_file(self):
        ...
        with self.assertRaisesRegex(MetadataFileError, "encoded as UTF-8 or ASCII"):
            Metadata.load(fp)
    ...
    def test_empty_file(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError, "locate header.*file may be empty"
        ):
            Metadata.load(fp)
    ...
    def test_comments_and_empty_rows_only(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError, "locate header.*only of comments or empty " "rows"
        ):
            Metadata.load(fp)
    ...
    def test_header_only_with_comments_and_empty_rows(self):
        fp = get_data_path("invalid/header-only-with-comments-and-empty-rows.tsv")
    ...
    def test_invalid_header(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError, "unrecognized ID column name.*" "invalid_id_header"
        ):
            Metadata.load(fp)
    ...
    def test_empty_column_name(self):
        ...
        with self.assertRaisesRegex(MetadataFileError, "column without a name"):
            Metadata.load(fp)
    ...
    def test_duplicate_ids(self):
        ...
        with self.assertRaisesRegex(MetadataFileError, "IDs must be unique.*id1"):
            Metadata.load(fp)
    ...
    def test_duplicate_column_names(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError, "Column names must be unique.*col1"
        ):
            Metadata.load(fp)

    def test_duplicate_column_names_with_whitespace(self):
        fp = get_data_path("invalid/duplicate-column-names-with-whitespace.tsv")
    ...
    def test_id_conflicts_with_id_header(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError, "ID 'id' conflicts.*ID column header"
        ):
            Metadata.load(fp)

    def test_column_name_conflicts_with_id_header(self):
        fp = get_data_path("invalid/column-name-conflicts-with-id-header.tsv")

        with self.assertRaisesRegex(
            MetadataFileError, "column name 'featureid' conflicts.*ID " "column header"
        ):
            Metadata.load(fp)
    ...
    def test_column_types_unrecognized_column_name(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError,
            "not_a_column.*column_types.*not a column " "in the metadata file",
        ):
    ...
    def test_column_types_unrecognized_column_type(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError,
            "col2.*column_types.*unrecognized column " "type.*CATEGORICAL",
        ):
            Metadata.load(fp, column_types={"col1": "numeric", "col2": "CATEGORICAL"})
    ...
    def test_directive_before_header(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError, "directive.*#q2:types.*searching for " "header"
        ):
            Metadata.load(fp)
    ...
    def test_unrecognized_directive(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError,
            "Unrecognized directive.*#q2:foo.*" "#q2:types.*#q2:missing.*directive",
        ):
            Metadata.load(fp)
    ...
    def test_duplicate_directives(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError, "duplicate directive.*#q2:types"
        ):
            Metadata.load(fp)
    ...
    def test_unrecognized_column_type_in_directive(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError,
            "col2.*unrecognized column type.*foo.*" "#q2:types directive",
        ):
            Metadata.load(fp)
    ...
    def test_column_types_directive_not_convertible_to_numeric(self):
        ...
        msg = (
            r"column 'col2' to numeric.*could not be interpreted as "
    ...
    def test_directive_after_directives_section(self):
        fp = get_data_path("invalid/directive-after-directives-section.tsv")

        with self.assertRaisesRegex(
            MetadataFileError, "#q2:types.*outside of the directives " "section"
        ):
            Metadata.load(fp)
    ...
    def test_directive_longer_than_header(self):
        ...
        with self.assertRaisesRegex(
            MetadataFileError, "row has 5 cells.*header declares 4 " "cells"
        ):
            Metadata.load(fp)
    ...
    def test_unknown_missing_scheme(self):
        ...
        with self.assertRaisesRegex(MetadataFileError, "col1.*BAD:SCHEME.*#q2:missing"):
            Metadata.load(fp)
...
class TestLoadSuccess(unittest.TestCase):
    def setUp(self):
        ...
        obs_columns = [
            (name, props.type) for name, props in self.simple_md.columns.items()
        ]
        exp_columns = [
            ("col1", "numeric"),
            ("col2", "categorical"),
    ...
    def test_retains_column_order(self):
        ...
        exp_columns = ["z", "y", "x"]
        exp_data = [[1.0, "a", "foo"], [2.0, "b", "bar"], [3.0, "c", "42"]]
    ...
    def test_recommended_ids(self):
        ...
        exp_index = pd.Index(
            ["c6ca034a-223f-40b4-a0e0-45942912a5ea", "My.ID"], name="id"
        )
    ...
    def test_non_standard_characters(self):
        ...
        exp_columns = ["↩c@l1™", "col(#2)", "#col'3", '"<col_4>"', "col\t  \r\n5"]
        exp_data = [
    ...
    def test_missing_data(self):
        ...
        obs_columns = [(name, props.type) for name, props in obs_md.columns.items()]
        exp_columns = [
            ("col1", "numeric"),
            ("NA", "numeric"),
            ("col3", "categorical"),
    ...
    def test_missing_insdc(self):
        ...
        exp_index = pd.Index(["id1", "id2", "id3", "id4", "id5", "id6"], name="id")
        exp_df = pd.DataFrame(
            {
                "col1": [1, 2, 3] + ([float("nan")] * 3),
                "col2": ["a", "b", "c"] + ([float("nan")] * 3),
                "col3": ["foo", "bar", "42", "anything", "whatever", "10"],
            },
            index=exp_index,
        )
        ...
        exp_md = Metadata(exp_df)
        pd.testing.assert_frame_equal(obs_md.to_dataframe(), exp_md.to_dataframe())

        obs_columns = [
            (name, props.type, props.missing_scheme)
    ...
    def test_insdc_override(self):
        ...
        exp_index = pd.Index(["id1", "id2", "id3", "id4"], name="id")
        exp_df = pd.DataFrame(
            {"col1": ["collected", "not collected", "not collected", "collected"]},
            index=exp_index,
        )
    ...
    def test_single_id(self):
        ...
        exp_df = pd.DataFrame(
            {"col1": [1.0], "col2": ["a"], "col3": ["foo"]}, index=exp_index
        )
        exp_md = Metadata(exp_df)
    ...
    def test_does_not_cast_ids_or_column_names(self):
        ...
        exp_index = pd.Index(
            ["0.000001", "0.004000", "0.000000"], dtype=object, name="id"
        )
    ...
    def test_column_types_override_directive(self):
        ...
        obs_md = Metadata.load(
            fp, column_types={"col1": "categorical", "col2": "categorical"}
        )
...
class TestSave(unittest.TestCase):
    ...
    def test_some_missing_data(self):
        md = Metadata(
            pd.DataFrame(
                {"col1": [42.0, np.nan, -3.5], "col2": ["a", np.nan, np.nan]},
                index=pd.Index(["id1", "id2", "id3"], name="id"),
            )
        )
    ...
    def test_missing_schemes(self):
        md = Metadata(
            pd.DataFrame(
                {
                    "col1": [42.0, np.nan, -3.5],
                    "col2": ["a", "not applicable", "restricted access"],
                },
                index=pd.Index(["id1", "id2", "id3"], name="id"),
            ),
            column_missing_schemes={"col1": "blank", "col2": "INSDC:missing"},
        )
    ...
    def test_default_missing_scheme_override(self):
        md = Metadata(
            ...
            column_missing_schemes=dict(col1="INSDC:missing", col2="INSDC:missing"),
        )
