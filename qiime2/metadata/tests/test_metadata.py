...
from qiime2.metadata import Metadata, CategoricalMetadataColumn, NumericMetadataColumn
...
class TestInvalidMetadataConstruction(unittest.TestCase):
    ...
    def test_no_ids(self):
        ...
        with self.assertRaisesRegex(ValueError, "Metadata.*at least one ID"):
            Metadata(pd.DataFrame({"column": []}, index=pd.Index([], name="id")))

    def test_invalid_id_header(self):
        # default index name
        with self.assertRaisesRegex(ValueError, r"Index\.name.*None"):
            Metadata(pd.DataFrame({"col": [1, 2, 3]}, index=pd.Index(["a", "b", "c"])))
    ...
    def test_non_str_id(self):
        with self.assertRaisesRegex(
            ...
            Metadata(
                pd.DataFrame(
                    {"col": [1, 2, 3]}, index=pd.Index(["a", np.nan, "c"], name="id")
                )
            )
    ...
    def test_duplicate_ids(self):
        with self.assertRaisesRegex(ValueError, "Metadata IDs.*unique.*'a'"):
            Metadata(
                pd.DataFrame(
                    {"col": [1, 2, 3]}, index=pd.Index(["a", "b", "a"], name="id")
                )
            )
    ...
    def test_duplicate_column_names(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Metadata column names.*unique.*'col1'"
        ):
            Metadata(
                pd.DataFrame(
                    data,
                    columns=["col1", "col2", "col1"],
                    index=pd.Index(["a", "b", "c"], name="id"),
                )
            )
    ...
    def test_unsupported_column_dtype(self):
        with self.assertRaisesRegex(
            ...
            Metadata(
                pd.DataFrame(
                    {"col1": [1, 2, 3], "col2": [True, False, True]},
                    index=pd.Index(["a", "b", "c"], name="id"),
                )
            )
    ...
    def test_categorical_column_empty_str(self):
        with self.assertRaisesRegex(
            ValueError, "CategoricalMetadataColumn.*empty strings.*" "column 'col2'"
        ):
            Metadata(
                pd.DataFrame(
                    {"col1": [1, 2, 3], "col2": ["foo", "", "bar"]},
                    index=pd.Index(["a", "b", "c"], name="id"),
                )
            )
    ...
    def test_numeric_column_infinity(self):
        with self.assertRaisesRegex(
            ValueError,
            "NumericMetadataColumn.*positive or negative " "infinity.*column 'col2'",
        ):
            Metadata(
                pd.DataFrame(
                    {"col1": ["foo", "bar", "baz"], "col2": [42, float("+inf"), 4.3]},
                    index=pd.Index(["a", "b", "c"], name="id"),
                )
            )
...
class TestMetadataConstructionAndProperties(unittest.TestCase):
    ...
    def test_single_id(self):
        ...
        self.assertEqualColumns(
            md.columns,
            [("col1", "numeric"), ("col2", "categorical"), ("col3", "categorical")],
        )
    ...
    def test_retains_column_order(self):
        ...
        self.assertEqualColumns(
            md.columns, [("z", "numeric"), ("a", "categorical"), ("ch", "categorical")]
        )
    ...
    def test_recommended_ids(self):
        ...
        self.assertEqual(md.column_count, 1)
        self.assertEqual(md.id_header, "id")
        self.assertEqual(md.ids, ("c6ca034a-223f-40b4-a0e0-45942912a5ea", "My.ID"))
    ...
    def test_missing_data_insdc(self):
            ...
            collections.OrderedDict(
                [
                    ...
                    (
                        "col4",
                        np.array(
                            [np.nan, np.nan, "restricted access", np.nan], dtype=object
                        ),
                    ),
                ]
            ),
        ...
        self.assertEqualColumns(
            md.columns,
            [("col1", "numeric"), ("col3", "categorical"), ("col4", "categorical")],
        )
    ...
    def test_does_not_cast_ids_or_column_names(self):
        ...
        self.assertEqualColumns(
            md.columns,
            [("42.0", "numeric"), ("1000", "categorical"), ("-4.2", "numeric")],
        )
    ...
    def test_case_insensitive_duplicate_column_names(self):
        index = pd.Index(["a", "b", "c"], name="id")
        df = pd.DataFrame(
            {"column": ["1", "2", "3"], "Column": ["4", "5", "6"]}, index=index
        )
        metadata = Metadata(df)
...
class TestSourceArtifacts(unittest.TestCase):
    def setUp(self):
        self.md = Metadata(
            pd.DataFrame({"col": [1, 2, 3]}, index=pd.Index(["a", "b", "c"], name="id"))
        )
...
class TestRepr(unittest.TestCase):
    def test_singular(self):
        md = Metadata(pd.DataFrame({"col1": [42]}, index=pd.Index(["a"], name="id")))
        ...
        self.assertIn(
            "col1: ColumnProperties(type='numeric'," " missing_scheme='blank')", obs
        )

    def test_plural(self):
        md = Metadata(
            pd.DataFrame(
                {"col1": [42, 42], "col2": ["foo", "bar"]},
        ...
        self.assertIn(
            "col2: ColumnProperties(type='categorical'," " missing_scheme='blank')", obs
        )
...
class TestEqualityOperators(unittest.TestCase, ReallyEqualMixin):
    ...
    def test_source_mismatch(self):
        ...
        pd.testing.assert_frame_equal(
            md_from_artifact.to_dataframe(), md_no_artifact.to_dataframe()
        )
    ...
    def test_id_mismatch(self):
        md1 = Metadata(
            pd.DataFrame({"a": "1", "b": "2"}, index=pd.Index(["0"], name="id"))
        )
        md2 = Metadata(
            pd.DataFrame({"a": "1", "b": "2"}, index=pd.Index(["1"], name="id"))
        )
    ...
    def test_column_name_mismatch(self):
        ...
        md2 = Metadata(
            pd.DataFrame({"a": "1", "c": "2"}, index=pd.Index(["0"], name="id"))
        )
    ...
    def test_column_type_mismatch(self):
        md1 = Metadata(
            pd.DataFrame(
                {"col1": ["42", "43"]}, index=pd.Index(["id1", "id2"], name="id")
            )
        )
        md2 = Metadata(
            pd.DataFrame({"col1": [42, 43]}, index=pd.Index(["id1", "id2"], name="id"))
        )
    ...
    def test_column_order_mismatch(self):
        ...
        md1 = Metadata(
            pd.DataFrame([[42, "foo"], [43, "bar"]], index=index, columns=["z", "a"])
        )
        md2 = Metadata(
            pd.DataFrame([["foo", 42], ["bar", 43]], index=index, columns=["a", "z"])
        )
    ...
    def test_data_mismatch(self):
        md1 = Metadata(
            pd.DataFrame({"a": "1", "b": "3"}, index=pd.Index(["0"], name="id"))
        )
        md2 = Metadata(
            pd.DataFrame({"a": "1", "b": "2"}, index=pd.Index(["0"], name="id"))
        )
    ...
    def test_equality_with_missing_data(self):
        md1 = Metadata(
            pd.DataFrame(
                {"col1": [1, np.nan, 4.2], "col2": [np.nan, "foo", np.nan]},
                index=pd.Index(["id1", "id2", "id3"], name="id"),
            )
        )
        md2 = Metadata(
...
class TestToDataframe(unittest.TestCase):
    ...
    def test_missing_data(self):
        ...
        self.assertEqual(
            obs.dtypes.to_dict(),
            {"col1": np.float64, "NA": object, "col3": object, "col4": object},
        )
    ...
    def test_dtype_int_normalized_to_dtype_float(self):
        index = pd.Index(["id1", "id2", "id3"], name="id")
        df = pd.DataFrame(
            {"col1": [42, -43, 0], "col2": [42.0, -43.0, 0.0], "col3": [42, np.nan, 0]},
            index=index,
        )

        self.assertEqual(
            df.dtypes.to_dict(),
            {"col1": np.int64, "col2": np.float64, "col3": np.float64},
        )
        ...
        self.assertEqual(
            obs.dtypes.to_dict(),
            {"col1": np.float64, "col2": np.float64, "col3": np.float64},
        )

    def test_encode_missing_no_missing(self):
        df = pd.DataFrame(
            {"col1": [42.0, 50.0], "col2": ["foo", "bar"]},
            index=pd.Index(["id1", "id2"], name="id"),
        )
        md = Metadata(df, default_missing_scheme="INSDC:missing")
    ...
    def test_insdc_missing_encode_missing_true(self):
        df = pd.DataFrame(
            {"col1": [42, "missing"], "col2": ["foo", "not applicable"]},
            index=pd.Index(["id1", "id2"], name="id"),
        )
        md = Metadata(df, default_missing_scheme="INSDC:missing")
    ...
    def test_insdc_missing_encode_missing_false(self):
        ...
        exp = pd.DataFrame(
            {"col1": [42, np.nan], "col2": ["foo", np.nan]},
            index=pd.Index(["id1", "id2"], name="id"),
        )
...
class TestGetColumn(unittest.TestCase):
    ...
    def test_column_name_not_found(self):
        ...
        with self.assertRaisesRegex(ValueError, "'col3'.*not a column.*'col1', 'col2'"):
            md.get_column("col3")
    ...
    def test_categorical_column(self):
        ...
        exp = CategoricalMetadataColumn(
            pd.Series(
                ["foo", "bar"], name="col2", index=pd.Index(["id1", "id2"], name="id")
            )
        )
    ...
    def test_numeric_column(self):
        ...
        exp = NumericMetadataColumn(
            pd.Series([42, 2.5], name="col1", index=pd.Index(["id1", "id2"], name="id"))
        )
    ...
    def test_id_header_preserved(self):
        ...
        exp = NumericMetadataColumn(
            pd.Series(
                [42, 2.5], name="col1", index=pd.Index(["a", "b"], name="#OTU ID")
            )
        )
...
class TestGetIDs(unittest.TestCase):
    ...
    def test_query_by_alternate_id_header(self):
        metadata = Metadata(
            pd.DataFrame({}, index=pd.Index(["id1", "id2", "id3"], name="#OTU ID"))
        )
...
class TestMerge(unittest.TestCase):
    ...
    def test_merging_nothing(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "At least one Metadata.*nothing to merge"
        ):
            md.merge()
    ...
    def test_merged_id_column_name(self):
        md1 = Metadata(
            pd.DataFrame(
                {"a": [1, 2]}, index=pd.Index(["id1", "id2"], name="sample ID")
            )
        )
        md2 = Metadata(
            pd.DataFrame(
                {"b": [3, 4]}, index=pd.Index(["id1", "id2"], name="feature ID")
            )
        )
        ...
        exp = Metadata(
            pd.DataFrame(
                {"a": [1, 2], "b": [3, 4]}, index=pd.Index(["id1", "id2"], name="id")
            )
        )
        self.assertEqual(obs, exp)
    ...
    def test_no_artifacts(self):
        md1 = Metadata(
            pd.DataFrame({"a": [1, 2]}, index=pd.Index(["id1", "id2"], name="id"))
        )
        md2 = Metadata(
            pd.DataFrame({"b": [3, 4]}, index=pd.Index(["id1", "id2"], name="id"))
        )
    ...
    def test_with_artifacts(self):
        ...
        md_no_artifact = Metadata(
            pd.DataFrame({"c": ["3", "42"]}, index=pd.Index(["0", "1"], name="id"))
        )
        ...
        exp_df = pd.DataFrame(
            {"a": "1", "b": "2", "c": "3", "d": "4"}, index=pd.Index(["0"], name="id")
        )
    ...
    def test_duplicate_columns(self):
        ...
        md2 = Metadata(
            pd.DataFrame(
                {"c": [5, 6], "b": [7, 8]}, index=pd.Index(["id1", "id2"], name="id")
            )
        )
...
class TestFilterIDs(unittest.TestCase):
    ...
    def test_empty_ids_to_keep(self):
        ...
        with self.assertRaisesRegex(ValueError, "ids_to_keep.*at least one ID"):
            md.filter_ids({})
    ...
    def test_duplicate_ids_to_keep(self):
        ...
        with self.assertRaisesRegex(ValueError, "ids_to_keep.*unique IDs.*'b'"):
            md.filter_ids(["b", "c", "b"])
    ...
    def test_missing_ids_to_keep(self):
        ...
        with self.assertRaisesRegex(ValueError, "IDs.*not present.*'d', 'id1'"):
...
class TestFilterColumns(unittest.TestCase):
    ...
    def test_all_filters_no_columns(self):
        md = Metadata(pd.DataFrame({}, index=pd.Index(["a", "b", "c"], name="id")))
    ...
    def test_all_columns_filtered(self):
        ...
        exp = Metadata(
            pd.DataFrame({}, index=pd.Index(["a", "b", "c", "d"], name="id"))
        )
    ...
    def test_retains_column_order(self):
        data = [[42, "foo", 2.5], [42, "bar", 0.5], [11, "foo", 0.0]]
        md = Metadata(
            pd.DataFrame(
                data,
                columns=["z", "a", "ch"],
    ...
    def test_alternate_id_header(self):
        md = Metadata(
            pd.DataFrame(
                {"col1": ["foo", "bar"], "col2": [-4.2, -4.2], "col3": ["bar", "baz"]},
