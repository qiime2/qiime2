...
class TestSourceArtifacts(unittest.TestCase):
    def setUp(self):
        self.mdc = DummyMetadataColumn(
            pd.Series([1, 2, 3], name="col", index=pd.Index(["a", "b", "c"], name="id"))
        )
    ...
    def test_add_duplicate_artifact(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "Duplicate source artifacts.*DummyMetadataColumn.*" "artifact: Mapping",
        ):
            self.mdc._add_artifacts([artifact1])
...
class TestEqualityOperators(unittest.TestCase, ReallyEqualMixin):
    ...
    def test_id_header_mismatch(self):
        mdc1 = DummyMetadataColumn(
            pd.Series([42, 43], name="col1", index=pd.Index(["id1", "id2"], name="id"))
        )
        mdc2 = DummyMetadataColumn(
            pd.Series([42, 43], name="col1", index=pd.Index(["id1", "id2"], name="ID"))
        )
    ...
    def test_id_mismatch(self):
        ...
        mdc2 = DummyMetadataColumn(
            pd.Series([42, 43], name="col1", index=pd.Index(["id1", "id3"], name="id"))
        )
    ...
    def test_column_name_mismatch(self):
        ...
        mdc2 = DummyMetadataColumn(
            pd.Series([42, 43], name="col2", index=pd.Index(["id1", "id2"], name="id"))
        )
    ...
    def test_data_mismatch(self):
        ...
        mdc2 = DummyMetadataColumn(
            pd.Series([42, 42], name="col1", index=pd.Index(["id1", "id2"], name="id"))
        )
...
class TestFilterIDs(unittest.TestCase):
    ...
    def test_supports_iterable(self):
        ...
        exp = DummyMetadataColumn(
            pd.Series([1, 3], name="col1", index=pd.Index(["a", "c"], name="id"))
        )
    ...
    def test_keep_one(self):
        ...
        exp = DummyMetadataColumn(
            pd.Series([2], name="col1", index=pd.Index(["b"], name="id"))
        )
    ...
    def test_alternate_id_header(self):
        mdc = DummyMetadataColumn(
            pd.Series(
                [1, 2, 3], name="col1", index=pd.Index(["a", "b", "c"], name="#OTU ID")
            )
        )
        ...
        exp = DummyMetadataColumn(
            pd.Series([2, 3], name="col1", index=pd.Index(["b", "c"], name="#OTU ID"))
        )
