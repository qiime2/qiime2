...
import qiime2.sdk
from qiime2.plugin.plugin import SemanticTypeRecord, FormatRecord, ArtifactClassRecord
...
from qiime2.core.testing.validator import (
...
class TestPluginManager(unittest.TestCase):
    ...
    def test_validators(self):
        self.assertEqual(
            {Kennel[Dog], Kennel[Cat], AscIntSequence, Squid, Octopus, Cuttlefish},
            set(self.pm.validators),
        )

        self.assertEqual(
            set([r.validator for r in self.pm.validators[Kennel[Dog]]._validators]),
        ...
        self.assertEqual(
            [r.validator for r in self.pm.validators[AscIntSequence]._validators],
            [validate_ascending_seq],
        )
    ...
    def test_type_fragments(self):
        ...
        exp = {
            "IntSequence1": SemanticTypeRecord(
                semantic_type=IntSequence1, plugin=self.plugin
            ),
            "IntSequence2": SemanticTypeRecord(
                semantic_type=IntSequence2, plugin=self.plugin
            ),
            "IntSequence3": SemanticTypeRecord(
                semantic_type=IntSequence3, plugin=self.plugin
            ),
            "Mapping": SemanticTypeRecord(semantic_type=Mapping, plugin=self.plugin),
            "FourInts": SemanticTypeRecord(semantic_type=FourInts, plugin=self.plugin),
            "Kennel": SemanticTypeRecord(semantic_type=Kennel, plugin=self.plugin),
            "Dog": SemanticTypeRecord(semantic_type=Dog, plugin=self.plugin),
            "Cat": SemanticTypeRecord(semantic_type=Cat, plugin=self.plugin),
            "SingleInt": SemanticTypeRecord(
                semantic_type=SingleInt, plugin=self.plugin
            ),
            "C1": SemanticTypeRecord(semantic_type=C1, plugin=self.plugin),
            "C2": SemanticTypeRecord(semantic_type=C2, plugin=self.plugin),
            "C3": SemanticTypeRecord(semantic_type=C3, plugin=self.plugin),
            "Foo": SemanticTypeRecord(semantic_type=Foo, plugin=self.plugin),
            "Bar": SemanticTypeRecord(semantic_type=Bar, plugin=self.plugin),
            "Baz": SemanticTypeRecord(semantic_type=Baz, plugin=self.plugin),
            "AscIntSequence": SemanticTypeRecord(
                semantic_type=AscIntSequence, plugin=self.plugin
            ),
            "Squid": SemanticTypeRecord(semantic_type=Squid, plugin=self.plugin),
            "Octopus": SemanticTypeRecord(semantic_type=Octopus, plugin=self.plugin),
            "Cuttlefish": SemanticTypeRecord(
                semantic_type=Cuttlefish, plugin=self.plugin
            ),
        }
    ...
    def test_get_semantic_types(self):
        ...
        kd = ArtifactClassRecord(
            semantic_type=Kennel[Dog],
        ...
        kc = ArtifactClassRecord(
            semantic_type=Kennel[Cat],
            format=MappingDirectoryFormat,
            plugin=self.plugin,
            description="",
            examples={},
    ...
    def test_get_formats_no_type_or_filter(self):
        exp = {
            "IntSequenceFormat": FormatRecord(
                format=IntSequenceFormat, plugin=self.plugin
            ),
            "IntSequenceDirectoryFormat": FormatRecord(
                format=IntSequenceDirectoryFormat, plugin=self.plugin
            ),
            "IntSequenceFormatV2": FormatRecord(
                format=IntSequenceFormatV2, plugin=self.plugin
            ),
            "IntSequenceV2DirectoryFormat": FormatRecord(
                format=IntSequenceV2DirectoryFormat, plugin=self.plugin
            ),
            "IntSequenceMultiFileDirectoryFormat": FormatRecord(
                format=IntSequenceMultiFileDirectoryFormat, plugin=self.plugin
            ),
            "RedundantSingleIntDirectoryFormat": FormatRecord(
                format=RedundantSingleIntDirectoryFormat, plugin=self.plugin
            ),
            "FourIntsDirectoryFormat": FormatRecord(
                format=FourIntsDirectoryFormat, plugin=self.plugin
            ),
            "EchoFormat": FormatRecord(format=EchoFormat, plugin=self.plugin),
            "EchoDirectoryFormat": FormatRecord(
                format=EchoDirectoryFormat, plugin=self.plugin
            ),
            "MappingDirectoryFormat": FormatRecord(
                format=MappingDirectoryFormat, plugin=self.plugin
            ),
            "Cephalapod": FormatRecord(format=Cephalapod, plugin=self.plugin),
            "CephalapodDirectoryFormat": FormatRecord(
                format=CephalapodDirectoryFormat, plugin=self.plugin
            ),
            "ImportableOnlyFormat": FormatRecord(
                format=ImportableOnlyFormat, plugin=self.plugin
            ),
            "ExportableOnlyFormat": FormatRecord(
                format=ExportableOnlyFormat, plugin=self.plugin
            ),
        }
    ...
    def test_get_formats_SFDF_EXPORTABLE(self):
        ...
        obs = self.pm.get_formats(
            filter=GetFormatFilters.EXPORTABLE, semantic_type=IntSequence1
        )
    ...
    def test_get_formats_SFDF_IMPORTABLE(self):
        ...
        obs = self.pm.get_formats(
            filter=GetFormatFilters.IMPORTABLE, semantic_type=IntSequence1
        )
    ...
    def test_get_formats_DF_EXPORTABLE(self):
        ...
        obs = self.pm.get_formats(
            filter=GetFormatFilters.EXPORTABLE, semantic_type=IntSequence3
        )
    ...
    def test_get_formats_DF_IMPORTABLE(self):
        ...
        obs = self.pm.get_formats(
            filter=GetFormatFilters.IMPORTABLE, semantic_type=IntSequence3
        )
