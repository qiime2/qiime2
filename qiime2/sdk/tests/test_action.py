...
class TestBadInputs(TestPluginBase):
    ...
    def test_artifact_passed_incorrectly(self):
        ...
        with self.assertRaisesRegex(TypeError, "int1.*type Int.*IntSequence1"):
        ...
        with self.assertRaisesRegex(TypeError, "ints3.*IntSequence2.*IntSequence1"):
    ...
    def test_primitive_passed_incorrectly(self):
        ...
        with self.assertRaisesRegex(TypeError, "ints2.*43.*incompatible.*IntSequence1"):
            concatenate_ints(ints1, arbitrary_int, ints3, int1, int2)

        # tests primitive passed as metadata
        with self.assertRaisesRegex(TypeError, "metadata.*43.*incompatible.*Metadata"):
            identity_with_metadata(ints1, arbitrary_int)

        # tests wrong type of primitive passed
        with self.assertRaisesRegex(
            TypeError, "age.*arbitraryString.*incompatible.*Int"
        ):
            params_only_method("key string", "arbitraryString")

        # tests metadata passed as artifact
        with self.assertRaisesRegex(TypeError, "'ints2'.*Metadata.*IntSequence1"):
    ...
    def test_primitive_param_out_of_range(self):
        ...
        # ... in a list
        with self.assertRaisesRegex(TypeError, "opt_nums.*-9999.*incompatible.*List"):
            range_nested_in_list(ints_list, int_set, nums, bad_range_val)

        # ... not in a list
        with self.assertRaisesRegex(
            TypeError, r"\'age\'.*-99999.*incompatible.*Int % Range\(0, None\)"
        ):
    ...
    def test_primitive_param_not_valid_choice(self):
        ...
        with self.assertRaisesRegex(TypeError, "break_from.*'invalid choice'"):
