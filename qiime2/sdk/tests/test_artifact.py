...
def get_data_path(filename):
    fp = qiime2.util.get_filepath_from_package("qiime2.sdk.tests", "data/%s" % filename)
    return str(fp)
...
class TestArtifact(unittest.TestCase, ArchiveTestingMixin):
    ...
    def test_private_constructor(self):
        with self.assertRaisesRegex(
            NotImplementedError, "Artifact constructor.*private.*Artifact.load"
        ):
            Artifact()
    ...
    def test_from_view(self):
        artifact = Artifact._from_view(
            FourInts, [-1, 42, 0, 43], list, self.provenance_capture
        )
    ...
    def test_from_view_union_not_valid(self):
        ...
        # Artifact._from_view.
        self.assertIsInstance(artifact.uuid, uuid.UUID)
        with self.assertRaisesRegex(Exception, "No transformation into either of"):
            self.assertEqual(artifact.view(Union[str, dict]), [-1, 42, 0, 43])

    def test_from_view_different_type_with_multiple_view_types(self):
        artifact = Artifact._from_view(
            IntSequence1, [42, 42, 43, -999, 42], list, self.provenance_capture
        )
        ...
        self.assertEqual(
            artifact.view(collections.Counter),
    ...
    def test_load_different_type_with_multiple_view_types(self):
        saved_artifact = Artifact.import_data(IntSequence1, [42, 42, 43, -999, 42])
        fp = os.path.join(self.test_dir.name, "artifact.qza")
        saved_artifact.save(fp)
    ...
    def test_import_data_invalid_type(self):
        with self.assertRaisesRegex(TypeError, "concrete semantic type.*Visualization"):
    ...
    def test_import_data_with_filepath_multi_file_data_layout(self):
        ...
        with self.assertRaisesRegex(
            qiime2.plugin.ValidationError, "FourIntsDirectoryFormat.*directory"
        ):
    ...
    def test_import_data_with_unreachable_path(self):
        with self.assertRaisesRegex(qiime2.plugin.ValidationError, "does not exist"):
            Artifact.import_data(
                IntSequence1, os.path.join(self.test_dir.name, "foo.txt")
            )

        with self.assertRaisesRegex(qiime2.plugin.ValidationError, "does not exist"):
            Artifact.import_data(FourInts, os.path.join(self.test_dir.name, "bar", ""))
    ...
    def test_cannot_be_viewed_as_metadata(self):
        A = Artifact.import_data("IntSequence1", [1, 2, 3, 4])
        with self.assertRaisesRegex(
            TypeError, "Artifact.*IntSequence1.*cannot be viewed " "as QIIME 2 Metadata"
        ):
            A.view(Metadata)
