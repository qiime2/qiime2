...
from qiime2.core.testing.type import FourInts, SingleInt, IntSequence1, IntSequence2
...
class TestResult(unittest.TestCase, ArchiveTestingMixin):
    ...
    def test_private_constructor(self):
        with self.assertRaisesRegex(
            NotImplementedError, "Result constructor.*private.*Result.load"
        ):
            Result()
    ...
    def test_import_data_single_dirfmt_to_single_dirfmt(self):
        ...
        qiime2.Artifact.import_data(
            "IntSequence2", temp_data_dir, view_type="IntSequenceDirectoryFormat"
        )
    ...
    def test_validate_artifact_bad(self):
        ...
        # for self and group
        set_permissions(artifact._archiver.root_dir, OTHER_NO_WRITE, OTHER_NO_WRITE)

        with (artifact._archiver.root_dir / "extra.file").open("w") as fh:
            fh.write("uh oh")

        with self.assertRaisesRegex(exceptions.ValidationError, r"extra\.file"):
            artifact.validate()
    ...
    def test_validate_vizualization_bad(self):
        ...
        # for self and group
        set_permissions(
            visualization._archiver.root_dir, OTHER_NO_WRITE, OTHER_NO_WRITE
        )
    ...
    def test_import_min_validate(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            _ = Artifact.import_data("IntSequence1", intseq_dir, validate_level="min")
...
class TestResultCollection(unittest.TestCase):
    ...
    def test_roundtrip_unordered_collection(self):
        ...
        with self.assertWarnsRegex(
            UserWarning,
            f"The directory '{self.output_fp}' does not " "contain a .order file",
        ):
            read_collection = ResultCollection.load(self.output_fp)

        self.assertEqual(set(self.collection.items()), set(read_collection.items()))

    def test_type_normal_collection(self):
        self.assertEqual(self.collection.type, qiime2.core.type.Collection[SingleInt])
    ...
    def test_type_weird_collection(self):
        ...
        self.assertEqual(
            ...
            qiime2.core.type.Collection[
                SingleInt | FourInts | IntSequence1 | IntSequence2
            ],
        )
    ...
    def test_collection_order_file_contains_nonexistent_key(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            f"The Result '{BAD_KEY}' is referenced in the "
