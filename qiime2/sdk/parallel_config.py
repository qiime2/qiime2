...
_TEST_CONFIG_ = {
    "parsl": {
        ...
        "executors": [
            ...
            {"class": "_TEST_EXECUTOR_", "label": "test", "max_threads": 1},
        ],
    }
}
...
def _get_vendored_config_path():
    ...
    if config_fp is None:
        ...
        if os.path.exists(
            fp_ := os.path.join(appdirs.user_config_dir("qiime2"), "qiime2_config.toml")
        ):
            config_fp = fp_
        ...
        # appdirs.site_config_dir(appname='qiime2, author='...')
            ...
            fp_ := os.path.join(appdirs.site_config_dir("qiime2"), "qiime2_config.toml")
        ):
            config_fp = fp_
...
class ParallelConfig:
    def __init__(self, parallel_config=None, action_executor_mapping={}):
        ...
        """
        if isinstance(parallel_config, str) or isinstance(parallel_config, os.PathLike):
    ...
    def __enter__(self):
        ...
        if PARALLEL_CONFIG.parallel_config is not None:
            raise ValueError(
                "ParallelConfig already loaded, cannot nest " "ParallelConfigs"
            )
        ...
        if not self.parallel_config:
            vendored_config, vendored_mapping, _ = get_vendored_config()
...
def _check_env(cls):
    if "QIIMETEST" not in os.environ:
        raise ValueError(f"Do not instantiate the class '{cls}' when not testing")
