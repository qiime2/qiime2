...
class TestIncompleteExp(unittest.TestCase):
    ...
    def test_repr(self):
        self.assertEqual(
            repr(self.IncompleteExp("Example", ("foo",))), "Example[{foo}]"
        )

        self.assertEqual(
            repr(self.IncompleteExp("Example", ("f", "b"))), "Example[{f}, {b}]"
        )
...
class TestTypeExp(unittest.TestCase):
    ...
    def test_repr(self):
        ...
        self.assertEqual(repr(Exclaim2[Face1, Exclaim0] % Face2), "![-_-, !] % (o_o)")
        self.assertEqual(repr(Face0 % Face2), "(o_-) % (o_o)")
        self.assertEqual(repr(Face0 % Face3), "(o_-) % (= & =)")
        self.assertEqual(repr(Exclaim2[Face1, Exclaim0] % Face3), "![-_-, !] % (= & =)")
        self.assertEqual(repr(Exclaim2[Face1, Exclaim0] % Face4), "![-_-, !] % (< | <)")
    ...
    def test_iter_symbols(self):
        ...
        self.assertEqual(
            {"Foo", "C2", "Bar"},
            set(
                (Foo | C2[Foo, Foo | C2[Foo % (P & Q) | Bar, Bar | Foo]]).iter_symbols()
            ),
        )
        ...
        self.assertEqual(
            (C2[Foo | Bar % P, Foo % (P & Q)] % (P | Q)).to_ast(),
            {
                ...
                "predicate": {
                    "type": "union",
                    "members": [
                        {"type": "predicate", "name": "P", "extra_junk": "P"},
                        {"type": "predicate", "name": "Q", "extra_junk": "Q"},
                    ],
                },
                "fields": [
                    {
...
class TestIntersection(unittest.TestCase):
    ...
    def test_distribution(self):
        ...
        self.assertTrue(((P | Q) & (R | S)).equals(P & R | P & S | Q & R | Q & S))
        ...
        self.assertTrue(
            (C2[Foo % P, Bar] & C2[Foo % Q, Bar]).equals(C2[Foo % (P & Q), Bar])
        )
...
class TestUnion(unittest.TestCase):
    ...
    def test_distribution(self):
        ...
        self.assertEqual(
            repr(Foo % P | Bar % Q | Foo % R | Bar % S), "Foo % (P | R) | Bar % (Q | S)"
        )

        self.assertEqual(
            repr(grammar.UnionExp([Foo % P, Bar % Q, Foo % R, Bar % S]).normalize()),
            "Foo % (P | R) | Bar % (Q | S)",
        )
...
class TestSubtyping(unittest.TestCase):
    ...
    def test_field(self):
        ...
        self.assertStrongSubtype(C2[Foo, Bar | Baz], Bar | C2[Foo, Foo | Bar | Baz])
    ...
    def test_generic_subtype(self):
        ...
        self.assertStrongSubtype(C1[C1[Foo]] | C1[C1[Bar]], C1[C1[Foo] | C1[Bar]])
