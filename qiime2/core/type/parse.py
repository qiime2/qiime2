...
def string_to_ast(type_expr):
    try:
        ...
        raise ValueError(
            "%r could not be parsed, it may not be a QIIME 2 type"
    ...
    try:
        ...
        raise ValueError(
            "Only one type expression may be parse at a time, got" ": %r" % (type_expr,)
        )
...
def _convert_literals(expr):
    ...
    if node is ast.Dict:
        return {
            _convert_literals(k): _convert_literals(v)
...
def ast_to_type(json_ast, scope=None):
    ...
    if type_ == "expression":
        ...
        if not json_ast["builtin"]:
            base_template = semantic.SemanticType(
                ...
                field_members={
                    ("field" + str(i)): [child] for i, child in enumerate(fields)
                },
            ).template
        ...
        return grammar.TypeExp(base_template, fields=fields, predicate=predicate)
    ...
    if type_ == "predicate":
        ...
        if name == "Properties":
            return semantic.Properties(json_ast["include"], exclude=json_ast["exclude"])
