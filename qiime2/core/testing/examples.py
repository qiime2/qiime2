...
def md1_factory():
    return Metadata(
        pd.DataFrame({"a": ["1", "2", "3"]}, index=pd.Index(["0", "1", "2"], name="id"))
    )


def md2_factory():
    return Metadata(
        pd.DataFrame({"b": ["4", "5", "6"]}, index=pd.Index(["0", "1", "2"], name="id"))
    )
...
def concatenate_ints_simple(use):
    ...
    (ints_d,) = use.action(
        use.UsageAction(plugin_id="dummy_plugin", action_id="concatenate_ints"),
        use.UsageInputs(ints1=ints_a, ints2=ints_b, ints3=ints_c, int1=4, int2=2),
...
def concatenate_ints_complex(use):
    ...
    (concatenated_ints,) = use.action(
        use.UsageAction(plugin_id="dummy_plugin", action_id="concatenate_ints"),
        use.UsageInputs(ints1=ints_d, ints2=ints_b, ints3=ints_c, int1=41, int2=0),
...
def typical_pipeline_complex(use):
    ...
    mapper2, ints2, *_ = use.action(
        use.UsageAction(plugin_id="dummy_plugin", action_id="typical_pipeline"),
        use.UsageInputs(int_sequence=ints1, mapping=mapper1, do_extra_thing=True),
        use.UsageOutputNames(
            out_map="out_map1",
            left="left1",
            right="right1",
    ...
    _, _, right2, *_ = use.action(
        use.UsageAction(plugin_id="dummy_plugin", action_id="typical_pipeline"),
        use.UsageInputs(int_sequence=ints2, mapping=mapper2, do_extra_thing=False),
        use.UsageOutputNames(
            out_map="out_map2",
            left="left2",
            right="right2",
...
def identity_with_metadata_simple(use):
    ...
    use.action(
        use.UsageAction(plugin_id="dummy_plugin", action_id="identity_with_metadata"),
...
def identity_with_metadata_column_get_mdc(use):
    ...
    (out,) = use.action(
        use.UsageAction(
            plugin_id="dummy_plugin", action_id="identity_with_metadata_column"
        ),
...
def variadic_input_simple(use):
    ...
    use.action(
        use.UsageAction(plugin_id="dummy_plugin", action_id="variadic_input_method"),
        use.UsageInputs(
            ints=[ints_a, ints_b], int_set={single_int1, single_int2}, nums={7, 8, 9}
        ),
...
def optional_inputs(use):
    ...
    (output1,) = use.action(
        use.UsageAction(
            plugin_id="dummy_plugin", action_id="optional_artifacts_method"
        ),
...
def collection_list_of_ints(use):
    ...
    (out,) = use.action(
        use.UsageAction(plugin_id="dummy_plugin", action_id="list_of_ints"),
...
def collection_of_visualizations(use):
    ...
    (vizualizations,) = use.action(
        use.UsageAction(plugin_id="dummy_plugin", action_id="viz_collection_pipeline"),
        use.UsageInputs(ints=ints),
        use.UsageOutputNames(visualizations="visualizations"),
    )
