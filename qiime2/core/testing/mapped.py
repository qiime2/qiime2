...
from qiime2.plugin import TypeMap, TypeMatch, Properties, Visualization, Bool, Choices


def constrained_input_visualization(output_dir: str, a: EchoFormat, b: EchoFormat):
...
def combinatorically_mapped_method(
    a: EchoFormat, b: EchoFormat
) -> (EchoFormat, EchoFormat):
    return a, b


T, R = TypeMap({Foo: Bar, Bar: Baz, Baz: Foo})
X, Y = TypeMap(
    {
...
def double_bound_variable_method(
    a: EchoFormat, b: EchoFormat, extra: EchoFormat
) -> EchoFormat:
    return extra
...
P = TypeMatch([Properties("A"), Properties("B"), Properties("C"), Properties("X", "Y")])
dummy_plugin.methods.register_function(
