...
def typical_pipeline(ctx, int_sequence, mapping, do_extra_thing, add=1):
    ...
    if do_extra_thing:
        left = ctx.make_artifact("IntSequence1", [i + add for i in left.view(list)])
...
def resumable_varied_pipeline(
    ctx, ints1, ints2, metadata, int1=None, string="None", fail=False
):
    varied_method = ctx.get_action("dummy_plugin", "varied_method")
    list_of_ints = ctx.get_action("dummy_plugin", "list_of_ints")
    dict_of_ints = ctx.get_action("dummy_plugin", "dict_of_ints")
    identity_with_metadata = ctx.get_action("dummy_plugin", "identity_with_metadata")
    most_common_viz = ctx.get_action("dummy_plugin", "most_common_viz")

    if int1 is None and string == "None":
        ints1_ret, ints2_ret, int1_ret = varied_method(ints1, ints2)
    else:
        ints1_ret, ints2_ret, int1_ret = varied_method(ints1, ints2, int1, string)
    ...
    return (ints1_ret, ints2_ret, int1_ret, list_ret, dict_ret, identity_ret, viz_ret)


# Either both int1 and string should be default or neither should be
def resumable_nested_varied_pipeline(
    ctx, ints1, ints2, metadata, int1=None, string="None", fail=False
):
    internal_pipeline = ctx.get_action("dummy_plugin", "internal_fail_pipeline")
    list_of_ints = ctx.get_action("dummy_plugin", "list_of_ints")
    dict_of_ints = ctx.get_action("dummy_plugin", "dict_of_ints")
    identity_with_metadata = ctx.get_action("dummy_plugin", "identity_with_metadata")
    most_common_viz = ctx.get_action("dummy_plugin", "most_common_viz")
    ...
    try:
        if int1 is None and string == "None":
            ints1_ret, ints2_ret, int1_ret = internal_pipeline(
                ints1, ints2, fail=fail
            )._result()
        else:
            ints1_ret, ints2_ret, int1_ret = internal_pipeline(
                ints1, ints2, int1, string, fail=fail
            )._result()
...
# Either both int1 and string should be default or neither should be
def internal_fail_pipeline(ctx, ints1, ints2, int1=None, string="None", fail=False):
    varied_method = ctx.get_action("dummy_plugin", "varied_method")
...
def list_pipeline(ctx, ints):
    ...
    return [ctx.make_artifact(SingleInt, 4), ctx.make_artifact(SingleInt, 5)]


def collection_pipeline(ctx, ints):
    assert isinstance(ints, ResultCollection)
    return {
        "key1": ctx.make_artifact(SingleInt, 4),
