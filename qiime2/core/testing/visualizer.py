...
def mapping_viz(
    output_dir: str, mapping1: dict, mapping2: dict, key_label: str, value_label: str
) -> None:
    ...
    with open(os.path.join(output_dir, "index.html"), "w") as fh:
        fh.write("<html><head>")
        fh.write('<link rel="stylesheet" type="text/css" ' 'href="css/style.css" />')
        fh.write("</head>\n")
        fh.write("<body>\n")
...
def _dict_to_dataframe(dict_, key_label, value_label):
    return pd.DataFrame(sorted(dict_.items()), columns=[key_label, value_label])
