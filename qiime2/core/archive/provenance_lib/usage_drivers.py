...
from qiime2.sdk.usage import Usage, UsageVariable, UsageInputs, UsageOutputs
...
def build_footer(dag: ProvDAG, boundary: str) -> List[str]:
    ...
    footer.append("# The following QIIME 2 Results were parsed to produce this script:")
    footer.extend(pairs)
    footer.append(boundary)
    footer.append("")
    return footer
...
class ReplayPythonUsage(ArtifactAPIUsage):
    ...
    copyright = importlib.resources.read_text(
        "qiime2.core.archive.provenance_lib.assets", "copyright_note.txt"
    ).split("\n")
    ...
    def __init__(
        self, enable_assertions: bool = False, action_collection_size: int = 2
    ):
        """
    ...
    def _template_action(
        ...
        lines = [f"{output_vars} = {plugin_id}_actions.{action_id}("]

        all_inputs = list(action_f.signature.inputs.keys()) + list(
            action_f.signature.parameters.keys()
        )
    ...
    def _template_outputs(self, action: Action, variables: UsageOutputs) -> str:
        """
    ...
    def render(self, flush: bool = False) -> str:
        ...
        rendered = "\n".join(self.header + sorted_imps + self.recorder + self.footer)
        if flush:
            self._reset_state()
        return rendered
