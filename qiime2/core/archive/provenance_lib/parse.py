...
from .archive_parser import Config, ParserResults, ProvNode, Parser, ArchiveParser
...
class ProvDAG:
    ...
    def terminal_uuids(self) -> Set[str]:
        ...
        self._terminal_uuids = {
            uuid for uuid, out_degree in cv.out_degree() if out_degree == 0
        }
        return self._terminal_uuids
    ...
    def union(cls, dags: List[ProvDAG]) -> ProvDAG:
        ...
        for next_dag in dags[1:]:
            union_dag._parsed_artifact_uuids = union_dag._parsed_artifact_uuids.union(
                next_dag._parsed_artifact_uuids
            )
            ...
            union_dag.cfg.parse_study_metadata = min(
                union_dag.cfg.parse_study_metadata, next_dag.cfg.parse_study_metadata
            )
            ...
            if union_dag.checksum_diff is None:
                ...
                union_dag.checksum_diff.added.update(next_dag.checksum_diff.added)
                union_dag.checksum_diff.removed.update(next_dag.checksum_diff.removed)
                union_dag.checksum_diff.changed.update(next_dag.checksum_diff.changed)
...
class DirectoryParser(Parser):
    accepted_data_types = "filepath to a directory containing .qza/.qzv archives"
    ...
    def get_parser(cls, artifact_data: Any) -> Parser:
        ...
        try:
            ...
            raise TypeError(f" in DirectoryParser: expects a directory, not a {t}.")

        if not is_dir:
            raise ValueError(
                f" in DirectoryParser: {artifact_data} " "is not a valid directory."
            )
...
class ProvDAGParser(Parser):
    ...
    def get_parser(cls, artifact_data: Any) -> Parser:
        ...
        if isinstance(artifact_data, ProvDAG):
            ...
            raise TypeError(f" in ProvDAGParser: {artifact_data} is not a ProvDAG.")
...
def select_parser(payload: Any) -> Parser:
    ...
    """
    _PARSER_TYPE_REGISTRY = [ArchiveParser, DirectoryParser, ProvDAGParser, EmptyParser]
