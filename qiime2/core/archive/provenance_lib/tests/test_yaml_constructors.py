...
class YamlConstructorTests(unittest.TestCase):
    ...
    def test_unknown_tag(self):
        ...
        with self.assertRaisesRegex(
            yaml.constructor.ConstructorError, "could not determine a constructor.*!foo"
        ):
            yaml.safe_load(tag)
    ...
    def test_metadata_path_constructor_one_Artifact_as_md(self):
        ...
        self.assertEqual(
            actual,
            MetadataInfo(
                ["415409a4-stuff-e3eaba5301b4"], "feature_metadata.tsv", md5sum_hash
            ),
        )
    ...
    def test_no_provenance_constructor(self):
        ...
        with self.assertWarnsRegex(UserWarning, "Artifact 34b07e.*prior to provenance"):
            actual = yaml.safe_load(tag)
