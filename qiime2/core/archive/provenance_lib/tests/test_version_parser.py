...
class TestVersionParser(unittest.TestCase):
    def setUp(self):
        ...
        self.tempdir = tempfile.mkdtemp(prefix="qiime2-test-version-parser-temp-")
    ...
    def test_parse_version_old_archive_format(self):
        ...
        with zipfile.ZipFile(fp) as zf:
            actual = parse_version(zf)
            self.assertEqual(actual, (archive_version_exp, self.framework_version_exp))
    ...
    def test_artifact_with_commit_version(self):
        ...
        with zipfile.ZipFile(fp) as zf:
            actual = parse_version(zf)
            self.assertEqual(actual, (self.archive_version_exp, framework_version_exp))
    ...
    def test_parse_version_no_VERSION_file(self):
        ...
        with zipfile.ZipFile(fp) as zf:
            with self.assertRaisesRegex(ValueError, "(?s)VERSION.*nonexistent.*"):
                parse_version(zf)
    ...
    splitvm = codecs.decode(_VERSION_MATCHER.encode("utf-8"), "unicode-escape").split(
        sep="\n"
    )
