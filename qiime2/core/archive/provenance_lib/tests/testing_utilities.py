...
class DummyArtifacts:
    ...
    def init_action_artifacts(self):
        ...
        identity_with_metadata_column = self.dp.actions["identity_with_metadata_column"]
        dict_of_ints = self.dp.actions["dict_of_ints"]
        optional_artifacts_method = self.dp.actions["optional_artifacts_method"]

        (concated_ints,) = concat_ints(
            self.int_seq1.artifact,
            self.int_seq1.artifact,
            self.int_seq2.artifact,
            7,
            13,
        )
        ...
        (int_seq_optional_input,) = optional_artifacts_method(self.int_seq1.artifact, 8)
    ...
    def init_no_checksum_dag(self):
        ...
        with warnings.catch_warnings():
            ...
            with generate_archive_with_file_removed(
                self.single_int.filepath, self.single_int.uuid, "checksums.md5"
            ) as altered_archive:
    ...
    def all_artifact_versions(self):
        return (
            self.table_v0,
            self.concated_ints_v1,
            self.concated_ints_v2,
            self.concated_ints_v3,
            self.concated_ints_v4,
            self.concated_ints_v5,
            self.concated_ints_v6,
        )
...
def write_zip_file(zfp, unzipped_dir):
    ...
    for root, dirs, files in os.walk(unzipped_dir):
        for file in files:
            ...
            zf.write(filepath, os.path.relpath(filepath, unzipped_dir))
    zf.close()


class CustomAssertions(unittest.TestCase):
    def assertREAppearsOnlyOnce(self, text, only_once, msg=None):
        appears_once_re = f"(?s)^(?:(?!{only_once}).)*{only_once}(?!.*{only_once}).*$"
...
def is_root_provnode_data(fp):
    ...
    if pathlib.Path(fp).parts[1] in ("VERSION", "metadata.yaml", "checksums.md5"):
        return True
