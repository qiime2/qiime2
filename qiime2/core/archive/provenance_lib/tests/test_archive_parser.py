...
from .testing_utilities import DummyArtifacts, is_root_provnode_data, write_zip_archive
    ...
    def test_parse_root_md_no_md_yaml(self):
        for artifact in self.das.all_artifact_versions:
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                metadata_path = os.path.join(tempdir, artifact.uuid, "metadata.yaml")
        ...
        for artifact in self.das.all_artifact_versions:
            ...
            if version == 0:
                with self.assertWarnsRegex(
                    UserWarning, "Artifact .*prior to provenance"
                ):
                ...
                self.assertIsInstance(res.prov_digraph, (type(None), nx.DiGraph))
    ...
    def test_correct_validate_checksums_method_called(self):
        ...
        for artifact in self.das.all_artifact_versions:
            ...
            if artifact.archive_version < 5:
                ...
                parser._validate_checksums = MagicMock(
                    return_value=(ValidationCode.VALID, ChecksumDiff({}, {}, {}))
                )
                parser.parse_prov(Config(), artifact.filepath)
                parser._validate_checksums.assert_called_once()
...
class ArchiveParserTests(unittest.TestCase):
    ...
    def test_get_parser(self):
        ...
        for artifact, parser_version in zip(self.das.all_artifact_versions, parsers):
            parser = ArchiveParser.get_parser(artifact.filepath)
...
class CitationsTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.zips = [os.path.join(cls.das.datadir, bib + ".zip") for bib in cite_strs]
    ...
    def test_many_citations(self):
        exp = [
            ...
            "unweighted_unifrac.+2",
            "unweighted_unifrac.+3",
            "unweighted_unifrac.+4",
            "BIOMV210DirFmt",
            "BIOMV210Format",
        ]
...
class ProvNodeTests(unittest.TestCase, ReallyEqualMixin):
    ...
    def setUpClass(cls):
        ...
        for artifact in cls.das.all_artifact_versions:
            with zipfile.ZipFile(artifact.filepath) as zf:
                ...
                cls.nodes[str(artifact.archive_version)] = ProvNode(
                    cfg, zf, root_md_fps
                )
        ...
        with zipfile.ZipFile(cls.das.concated_ints_with_md.filepath) as zf:
            ...
            node_fps = [
                pathlib.Path(fp)
                for fp in all_filenames
                if non_md_node_id in fp
                and ("metadata.yaml" in fp or "action.yaml" in fp or "VERSION" in fp)
            ]
    ...
    def test_node_properties(self):
        ...
        for node, archive_version in zip(self.nodes, [str(i) for i in range(7)]):
            if archive_version == "0":
                self.assertEqual(self.nodes[node].format, "BIOMV210DirFmt")
                self.assertEqual(self.nodes[node].type, "FeatureTable[Frequency]")
                self.assertEqual(self.nodes[node].has_provenance, False)
            else:
                self.assertEqual(self.nodes[node].format, "IntSequenceDirectoryFormat")
            ...
            self.assertEqual(
                self.nodes[node].framework_version, framework_versions[archive_version]
            )
    ...
    def test_str(self):
        for node_vzn, artifact in zip(self.nodes, self.das.all_artifact_versions):
            uuid = artifact.uuid
            self.assertRegex(
                str(self.nodes[node_vzn]), f"(?s)UUID:\t\t{uuid}.*Type.*Data Format"
            )
    ...
    def test_repr(self):
        for node_vzn, artifact in zip(self.nodes, self.das.all_artifact_versions):
            uuid = artifact.uuid
            self.assertRegex(
                repr(self.nodes[node_vzn]), f"(?s)UUID:\t\t{uuid}.*Type.*Data Format"
            )
    ...
    def test_get_metadata_from_action(self):
        ...
        md = MetadataInfo(["d5b4cf78-f5e2-44e0-aa24-66b02564e9f1"], "metadata.tsv", "0")
        action_details = {"parameters": [{"metadata": md}]}
        all_md, artifacts_as_md = find_md(action_details)
        all_exp = {"metadata": "metadata.tsv"}
        a_as_md_exp = [
            {"artifact_passed_as_metadata": "d5b4cf78-f5e2-44e0-aa24-66b02564e9f1"}
        ]
    ...
    def test_metadata_available_in_property(self):
        ...
        self.assertEqual(type(self.nonroot_md_node.metadata["metadata"]), pd.DataFrame)
    ...
    def test_metadata_is_correct(self):
        ...
        pd.testing.assert_frame_equal(md_exp, self.nonroot_md_node.metadata["metadata"])
    ...
    def test_parents_for_import_node(self):
        ...
        with zipfile.ZipFile(self.das.single_int.filepath) as zf:
            ...
            import_node_fps = [
                ...
                if uuid in fp and any(map(lambda x: x in fp, required_fps))
            ]
