...
from ..replay import (
    ...
    group_by_action,
    init_md_from_artifacts,
    init_md_from_md_file,
...
class ReplayNamespacesTests(unittest.TestCase):
    ...
    def test_add_usage_var_workflow(self):
        ...
        self.assertIsInstance(ns.get_usg_var_record(uuid).variable, UsageVariable)
        self.assertEqual(ns.get_usg_var_record(uuid).name, exp_name)
...
class ReplayProvenanceTests(unittest.TestCase):
    ...
    def test_replay_from_fp(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            replay_provenance(ReplayPythonUsage, in_fn, out_fn, md_out_dir=tmpdir)
            ...
            self.assertRegex(rendered, "The following command.*additional metadata")
            self.assertIn("mapping_0.view(Metadata)", rendered)
            self.assertIn("dummy_plugin_actions.identity_with_metadata", rendered)
    ...
    def test_replay_use_md_without_dump_md(self):
        in_fp = self.das.concated_ints.filepath
        with self.assertRaisesRegex(
            NotImplementedError, "(?s)uses.*metadata.*must.*written to disk"
        ):
            replay_provenance(
                ReplayPythonUsage,
                in_fp,
    ...
    def test_replay_from_provdag(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            replay_provenance(ReplayPythonUsage, dag, out_fn, md_out_dir=tmpdir)
    ...
    def test_replay_from_provdag_use_md_without_parse(self):
        dag = ProvDAG(
            self.das.concated_ints_with_md.filepath,
            validate_checksums=False,
            parse_metadata=False,
        )
        with self.assertRaisesRegex(ValueError, "Metadata not parsed for replay"):
            replay_provenance(
    ...
    def test_replay_from_provdag_ns_collision(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            self.das.concated_ints.artifact.save(os.path.join(tempdir, "c1.qza"))
            self.das.other_concated_ints.artifact.save(os.path.join(tempdir, "c2.qza"))
            dag = ProvDAG(tempdir)

        exp = ["concatenated_ints_0", "concatenated_ints_1"]
        with tempfile.TemporaryDirectory() as tempdir:
            out_path = pathlib.Path(tempdir) / "ns_coll.txt"
            replay_provenance(ReplayPythonUsage, dag, out_path, md_out_dir=tempdir)
...
class MultiplePluginTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        (concated_ints,) = concat_ints(int_seq, int_seq, int_seq, 5, 6)
    ...
    def test_multiple_plugins_in_provenance(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            replay_provenance(ReplayPythonUsage, fp, out_fp, md_out_dir=tempdir)
        ...
        self.assertIn(
            "import qiime2.plugins.other_plugin.actions as " "other_plugin_actions",
            rendered,
        )
        self.assertIn("dummy_plugin_actions.split_ints(", rendered)
        self.assertIn("other_plugin_actions.concatenate_ints(", rendered)
    ...
    def test_directory_replay_multiple_imports(self):
        ...
        for artifact in self.das.single_int, self.das.single_int2:
            for dir_ in inner_dir, outer_dir:
                artifact.artifact.save(os.path.join(dir_, f"{artifact.name}.qza"))
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            replay_provenance(ReplayPythonUsage, dir_dag, out_path, md_out_dir=tempdir)
...
class BuildUsageExamplesTests(unittest.TestCase):
    ...
    def test_build_usage_examples(self, n_p_builder, imp_builder, act_builder):
        ns = ReplayNamespaces()
        dag = self.das.concated_ints_with_md.dag
        cfg = ReplayConfig(use=ReplayPythonUsage(), use_recorded_metadata=False)
...
class MiscHelperFnTests(unittest.TestCase):
    ...
    def test_dump_recorded_md_file_no_md(self):
        ...
        with self.assertRaisesRegex(ValueError, "should only be called.*if.*metadata"):
...
class InitializerTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with zipfile.ZipFile(cls.das.concated_ints_with_md_column.filepath) as zf:
    ...
    def test_init_md_from_md_file(self):
        ...
        self.assertIn("from qiime2 import Metadata", rendered)
        self.assertIn(
            "metadata_0_md = Metadata.load(<your metadata filepath>)", rendered
        )
    ...
    def test_init_md_from_recorded_md(self):
        ...
        with self.assertRaisesRegex(ValueError, "only.*call.*if.*metadata"):
            init_md_from_recorded_md(no_md_node, param_name, var_name, ns, cfg, md_fn)

        var = init_md_from_recorded_md(md_node, param_name, var_name, ns, cfg, md_fn)
        self.assertIsInstance(var, UsageVariable)
        self.assertEqual(var.var_type, "metadata")
        ...
        self.assertIn("recorded_metadata/identity_with_metadata/metadata_0", rendered)
    ...
    def test_init_md_from_recorded_mdc(self):
        ...
        self.assertIn(
            "recorded_metadata/identity_with_metadata_column/" "metadata_0.tsv",
            rendered,
        )
...
class BuildNoProvenanceUsageTests(CustomAssertions):
    ...
    def test_build_no_provenance_node_usage_many(self):
        ...
        self.assertEqual(ns._usg_var_ns[uuid].name, "feature_table_frequency_0")
        self.assertEqual(
...
class BuildActionUsageTests(CustomAssertions):
    ...
    def test_build_action_usage_recorded_md(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            cfg = ReplayConfig(
                use=ReplayPythonUsage(), use_recorded_metadata=False, md_out_dir=tempdir
            )
            ...
            ns.add_usg_var_record(mapping_import_uuid, "imported_mapping", mapping_var)
            ns.add_usg_var_record(intseq_import_uuid_1, "imported_ints", intseq_var_1)
            ns.add_usg_var_record(intseq_import_uuid_2, "imported_ints", intseq_var_2)
