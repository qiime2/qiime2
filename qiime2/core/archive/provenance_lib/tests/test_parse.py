...
class ProvDAGTests(unittest.TestCase):
    ...
    def test_number_of_terminal_nodes(self):
        ...
        num_concated_ints_term_nodes = len(self.das.concated_ints.dag.terminal_nodes)
        ...
        dir_fp = self.save_artifact_to_dir(
            self.das.concated_ints, "int-and-concated-ints"
        )
        dag = ProvDAG(dir_fp)
        self.assertEqual(len(dag.terminal_nodes), 1)

    def test_number_of_terminal_nodes_pipeline(self):
        num_pipeline_viz_term_nodes = len(self.das.pipeline_viz.dag.terminal_nodes)
    ...
    def test_node_action_names(self):
        ...
        self.assertEqual(concated_ints_node.action.action_name, "concatenate_ints")

    def test_node_action_names_pipeline(self):
        pipeline_viz_node, *_ = self.das.pipeline_viz.dag.terminal_nodes
        self.assertEqual(pipeline_viz_node.action.action_name, "typical_pipeline")

    def test_has_correct_edges(self):
        edges = self.das.concated_ints.dag.dag.edges

        self.assertIn((self.das.int_seq1.uuid, self.das.concated_ints.uuid), edges)
        self.assertIn((self.das.int_seq2.uuid, self.das.concated_ints.uuid), edges)
        self.assertNotIn((self.das.int_seq1.uuid, self.das.int_seq2.uuid), edges)
        self.assertNotIn((self.das.concated_ints.uuid, self.das.int_seq1.uuid), edges)
    ...
    def test_captures_full_history(self):
        ...
        for _ in range(iterations):
            (next_concated_ints,) = concat_ints(
                next_concated_ints, next_concated_ints, self.das.int_seq2.artifact, 4, 6
            )
    ...
    def test_get_outer_provenance_nodes(self):
        ...
        os.mkdir(fp)
        self.das.concated_ints.artifact.save(os.path.join(fp, "concated-ints.qza"))
    ...
    def test_invalid_provenance(self):
        ...
        with generate_archive_with_file_removed(
            self.das.int_seq1.filepath, uuid, os.path.join("data", "ints.txt")
        ) as altered_archive:
            ...
            self.assertEqual(list(diff.changed.keys()), ["provenance/citations.bib"])

    def test_missing_checksums_md5(self):
        uuid = self.das.single_int.uuid
        with generate_archive_with_file_removed(
            self.das.single_int.filepath, uuid, "checksums.md5"
        ) as altered_archive:
            expected = (
    ...
    def test_v0_archive(self):
        ...
        self.assertEqual(dag.provenance_is_valid, ValidationCode.PREDATES_CHECKSUMS)
    ...
    def test_union_identity(self):
        ...
        self.assertRegex(
            repr(unioned_dag), f"ProvDAG representing the provenance.*Artifacts.*{uuid}"
        )

    def test_union_two(self):
        unioned_dag = ProvDAG.union([self.das.single_int.dag, self.das.int_seq2.dag])
        ...
        self.assertRegex(rep, "ProvDAG representing the provenance.*Artifacts.")
        self.assertRegex(rep, f"{self.das.single_int.uuid}")
        self.assertRegex(rep, f"{self.das.int_seq2.uuid}")

        self.assertEqual(nx.number_weakly_connected_components(unioned_dag.dag), 2)

    def test_union_many(self):
        unioned_dag = ProvDAG.union(
            [self.das.single_int.dag, self.das.int_seq1.dag, self.das.mapping1.dag]
        )
        self.assertEqual(
            {self.das.single_int.uuid, self.das.int_seq1.uuid, self.das.mapping1.uuid},
            unioned_dag._parsed_artifact_uuids,
        )
        self.assertEqual(unioned_dag.provenance_is_valid, ValidationCode.VALID)
        ...
        self.assertEqual(nx.number_weakly_connected_components(unioned_dag.dag), 3)

    def test_union_self_missing_checksums_md5(self):
        unioned_dag = ProvDAG.union([self.das.dag_missing_md5, self.das.single_int.dag])
        ...
        self.assertEqual(unioned_dag.provenance_is_valid, ValidationCode.INVALID)

        self.assertEqual(nx.number_weakly_connected_components(unioned_dag.dag), 1)
    ...
    def test_union_other_missing_checksums_md5(self):
        ...
        """
        unioned_dag = ProvDAG.union([self.das.single_int.dag, self.das.dag_missing_md5])
    ...
    def test_union_v0_v1_archives(self):
        ...
        self.assertFalse(unioned_dag.node_has_provenance(self.das.table_v0.uuid))
        self.assertTrue(unioned_dag.node_has_provenance(self.das.concated_ints_v2.uuid))
    ...
    def test_union_v3_v5_archives(self):
        ...
        self.assertTrue(unioned_dag.node_has_provenance(self.das.concated_ints_v3.uuid))
        self.assertTrue(unioned_dag.node_has_provenance(self.das.concated_ints_v5.uuid))
    ...
    def test_union_v5_v6_archives(self):
        ...
        self.assertTrue(unioned_dag.node_has_provenance(self.das.concated_ints_v5.uuid))
        self.assertTrue(unioned_dag.node_has_provenance(self.das.concated_ints_v6.uuid))
    ...
    def test_dag_is_superset(self):
        ...
        """
        unioned_dag = ProvDAG.union(
            [self.das.concated_ints.dag, self.das.int_seq1.dag, self.das.int_seq2.dag]
        )

        self.assertIn(self.das.int_seq1.uuid, unioned_dag._parsed_artifact_uuids)
        ...
        self.assertEqual(unioned_dag.terminal_uuids, {self.das.concated_ints.uuid})
    ...
    def test_three_artifacts_two_terminal_uuids(self):
        ...
        self.assertIn(self.das.concated_ints.uuid, unioned_dag._parsed_artifact_uuids)
        self.assertIn(
            self.das.other_concated_ints.uuid, unioned_dag._parsed_artifact_uuids
        )
    ...
    def test_one_analysis_two_artifacts(self):
        ...
        """
        unioned_dag = ProvDAG.union(
            [self.das.concated_ints.dag, self.das.other_concated_ints.dag]
        )
    ...
    def test_no_checksum_validation_on_intact_artifact(self):
        ...
        self.assertEqual(
            no_validation_dag.provenance_is_valid, ValidationCode.VALIDATION_OPTOUT
        )
    ...
    def test_no_checksum_validation_missing_checksums_md5(self):
        with generate_archive_with_file_removed(
            ...
            self.assertEqual(dag.provenance_is_valid, ValidationCode.VALIDATION_OPTOUT)
...
class EmptyParserTests(unittest.TestCase):
    ...
    def test_get_parser_input_data_not_none(self):
        ...
        with open(fp, "w") as fh:
            fh.write("some text\n")
        with self.assertRaisesRegex(TypeError, f"EmptyParser.*{fn} is not None"):
...
class ProvDAGParserTests(unittest.TestCase):
    ...
    def test_get_parser_input_data_not_a_provdag(self):
        ...
        with self.assertRaisesRegex(TypeError, f"ProvDAGParser.*{fn} is not a ProvDAG"):
    ...
    def test_parse_a_provdag(self):
        ...
        for archive in self.das.all_artifact_versions:
            ...
            self.assertIsInstance(parsed, ParserResults)
            self.assertEqual(parsed.parsed_artifact_uuids, dag._parsed_artifact_uuids)
            self.assertTrue(nx.is_isomorphic(parsed.prov_digraph, dag.dag))
            self.assertEqual(parsed.provenance_is_valid, dag.provenance_is_valid)
...
class SelectParserTests(unittest.TestCase):
    ...
    def test_correct_archive_parser_version(self):
        parsers = [ParserV0, ParserV1, ParserV2, ParserV3, ParserV4, ParserV5, ParserV6]
...
class ParseProvenanceTests(unittest.TestCase):
    ...
    def test_parse_with_artifact_parser(self):
        ...
        self.assertIsInstance(
            parser_results.prov_digraph.nodes[uuid]["node_data"], ProvNode
        )
        self.assertEqual(parser_results.provenance_is_valid, ValidationCode.VALID)
        self.assertEqual(parser_results.checksum_diff, ChecksumDiff({}, {}, {}))
    ...
    def test_parse_with_directory_parser(self):
        ...
        self.assertEqual(res.parsed_artifact_uuids, {concated_ints_uuid, mapping_uuid})
...
class DirectoryParserTests(unittest.TestCase):
    ...
    def test_directory_parser_multiple_imports(self):
        ...
        shutil.copy(
            self.das.mapping1.filepath, os.path.join(outer_path, "mapping1.qza")
        )
        shutil.copy(
            self.das.mapping2.filepath, os.path.join(outer_path, "mapping2.qza")
        )
        shutil.copy(
    ...
    def test_verbose(self):
        ...
        with redirect_stdout(buffer):
            dag = ProvDAG(
                os.path.join(self.tempdir, "parse-dir"), verbose=True, recurse=True
            )
