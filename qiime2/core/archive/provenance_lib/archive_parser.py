...
from ._checksum_validator import ValidationCode, ChecksumDiff, validate_checksums
...
class ProvNode:
    ...
    def __init__(self, cfg: Config, zf: ZipFile, node_fps: List[pathlib.Path]):
        """
        ...
        for fp in node_fps:
            if fp.name == "VERSION":
                self._archive_version, self._framework_version = parse_version(zf)
        ...
        if (all_params := action_details.get("parameters")) is not None:
            for param in all_params:
                ...
                if isinstance(param_val, MetadataInfo):
                    ...
                    artifacts_as_metadata += [
                        {"artifact_passed_as_metadata": uuid}
                        for uuid in param_val.input_artifact_uuids
                    ]
...
class ParserV0(ArchiveParser):
    ...
    def parse_prov(self, cfg: Config, archive: str) -> ParserResults:
        ...
        with ZipFile(archive) as zf:
            if cfg.perform_checksum_validation:
                provenance_is_valid, checksum_diff = self._validate_checksums(zf)
            else:
            ...
            self._assert_expected_files_present(zf, exp_node_fps, prov_fps)
        ...
        return ParserResults({root_uuid}, graph, provenance_is_valid, checksum_diff)
    ...
    def _digraph_from_archive_contents(
        ...
        for node_uuid, node in archive_contents.items():
            node_info = {"node_data": node, "has_provenance": node.has_provenance}
    ...
    def _assert_expected_files_present(
        ...
        for fp in expected_node_fps:
            if fp not in prov_fps:
                ...
                error_contents += (
                    f"Archive {root_uuid} may be corrupt " "or provenance may be false."
                )
...
class ParserV2(ParserV1):
    ...
    expected_files_all_nodes = (
        *ParserV1.expected_files_all_nodes,
        "action/action.yaml",
    )
...
class ParserV4(ParserV3):
    ...
    expected_files_all_nodes = (*ParserV3.expected_files_all_nodes, "citations.bib")
