...
class TestProvenanceIntegration(unittest.TestCase):
    ...
    def test_chain_with_artifact_metadata(self):
        ...
        metadata_artifact_2 = qiime2.Artifact.import_data("Mapping", {"c": "baz"})
        ...
        with (
            p_dir / "artifacts" / str(b.uuid) / "action" / "action.yaml"
        ).open() as fh:
    ...
    def test_chain_with_merged_artifact_metadata(self):
        md_artifact1 = qiime2.Artifact.import_data("Mapping", {"a": "foo", "b": "bar"})
        md_artifact2 = qiime2.Artifact.import_data("Mapping", {"c": "baz"})
        ...
        c = dummy_plugin.actions.identity_with_metadata_column(b, merged_mdc).out

        p_dir = c._archiver.provenance_dir

        yaml_value = "%s,%s:metadata.tsv" % (md_artifact1.uuid, md_artifact2.uuid)
        ...
        # Check that metadata is written out fully
        with (p_dir / "action" / "metadata.tsv").open() as fh:
            self.assertEqual(fh.read(), "id\tc\n#q2:types\tcategorical\n0\tbaz\n")

        new_merged_md = qiime2.Metadata.load(
            str(p_dir / "artifacts" / str(b.uuid) / "action" / "metadata.tsv")
        )
        pd.testing.assert_frame_equal(
            new_merged_md.to_dataframe(), merged_md.to_dataframe()
        )

        # Check that provenance of originating metadata artifacts exists
        self.assertTrue(
            (
                p_dir / "artifacts" / str(md_artifact1.uuid) / "action" / "action.yaml"
            ).exists()
        )
        self.assertTrue(
            (
                p_dir / "artifacts" / str(md_artifact2.uuid) / "action" / "action.yaml"
            ).exists()
        )
    ...
    def test_with_optional_artifacts(self):
        ...
        self.assertTrue(
            (p_dir / "artifacts" / str(ints1.uuid) / "action" / "action.yaml").exists()
        )
        self.assertTrue(
            (p_dir / "artifacts" / str(ints2.uuid) / "action" / "action.yaml").exists()
        )
    ...
    def test_nested_pipeline_alias_of(self):
        ...
        match = re.search(
            r"alias\-of: ([a-zA-Z0-9\-]+)$", right_yaml, flags=re.MULTILINE
        )
        first_alias = match.group(1)

        with (
            right_p_dir / "artifacts" / first_alias / "action" / "action.yaml"
        ).open() as fh:
        ...
        match = re.search(
            r"alias\-of: ([a-zA-Z0-9\-]+)$", first_alias_yaml, flags=re.MULTILINE
        )

        second_alias = match.group(1)

        with (
            right_p_dir / "artifacts" / second_alias / "action" / "action.yaml"
        ).open() as fh:
    ...
    @mock.patch(
        "qiime2.core.archive.provenance.tzlocal.get_localzone", side_effect=ValueError()
    )
    ...
    @mock.patch("qiime2.core.path.ProvenancePath.rename", side_effect=FileExistsError)
