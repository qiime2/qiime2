...
class TestFindDuplicates(unittest.TestCase):
    ...
    def test_multiple_duplicates(self):
        obs = util.find_duplicates(iter(["foo", "bar", "foo", "baz", "foo", "bar"]))

        self.assertEqual(obs, {"foo", "bar"})

    def test_all_duplicates(self):
        obs = util.find_duplicates(iter(["foo", "bar", "baz", "baz", "bar", "foo"]))
...
class TestDurationTime(unittest.TestCase):
    ...
    def test_singular(self):
        ...
        self.assertEqual(
            util.duration_time(reldelta),
            "1 year, 1 month, 1 day, 1 hour, 1 minute, 1 second," " and 1 microsecond",
        )
...
class TestMD5Sum(unittest.TestCase):
    ...
    def test_empty_file(self):
        self.assertEqual(
            util.md5sum(self.make_file(b"")), "d41d8cd98f00b204e9800998ecf8427e"
        )

    def test_single_byte_file(self):
        self.assertEqual(
            util.md5sum(self.make_file(b"a")), "0cc175b9c0f1b6a831c399e269772661"
        )

    def test_large_file(self):
        path = self.make_file(b"verybigfile" * (1024 * 50))
        self.assertEqual(util.md5sum(path), "27d64211ee283283ad866c18afa26611")
...
class TestMD5SumDirectory(unittest.TestCase):
    ...
    def test_empty_directory(self):
        self.assertEqual(
            util.md5sum_directory(self.test_path), collections.OrderedDict()
        )
    ...
    @pytest.mark.skipif(
        shutil.which("md5sum") is None, reason="md5sum executable is required"
    )
...
class TestChecksumFormat(unittest.TestCase):
    def test_to_simple(self):
        line = util.to_checksum_format(
            "this/is/a/filepath", "d9724aeba59d8cea5265f698b2c19684"
        )
        self.assertEqual(line, "d9724aeba59d8cea5265f698b2c19684  this/is/a/filepath")
    ...
    def test_to_hard(self):
        ...
        line = util.to_checksum_format(
            "filepath/\n/with/\\newline", "939aaaae6098ebdab049b0f3abe7b68c"
        )

        # Note raw string
        self.assertEqual(
            line, r"\939aaaae6098ebdab049b0f3abe7b68c  filepath/\n/with/\\newline"
        )
