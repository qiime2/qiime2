...
def _fake_user_for_cache(cache_prefix, i_acknowledge_this_is_dangerous=False):
    ...
    if not i_acknowledge_this_is_dangerous:
        raise ValueError(
            "YOU MUST ACCEPT THE DANGER OF LETTING THIS SCRIPT "
            "MAKE AND REMOVE A USER"
        )

    if not os.getegid() == 0:
        raise ValueError(
            "This action requires super user permissions which " "you do not have"
        )
...
def _load_outputs(collection):
    ...
    for result in collection.values():
        output = load_action_yaml(result._archiver.path)["action"]["output-name"]
...
class TestCache(unittest.TestCase):
    ...
    def test_remove(self):
        ...
        with self.assertRaisesRegex(
            KeyError, f"'{self.cache.path}' does not contain the key " "'foo'"
        ):
            self.cache.load("foo")
    ...
    def test_garbage_collection(self):
        ...
        expected_pre_gc_contents = set(
            (
                ...
                f"data/{self.art1.uuid}",
                f"data/{self.art2.uuid}",
                f"data/{self.art3.uuid}",
                f"data/{self.art4.uuid}",
            )
        )

        expected_post_gc_contents = set(
            (
                "./VERSION",
                "keys/foo",
                "keys/bar",
    ...
    def test_asynchronous(self):
        ...
        with self.cache:
            future = concatenate_ints.asynchronous(
                self.art1, self.art2, self.art4, 4, 5
            )
    ...
    def test_pool_wrong_cache_set(self):
        ...
        with cache:
            with self.assertRaisesRegex(
                ValueError,
                "pool that is not on the currently " f"set cache.*{cache.path}",
            ):
                with pool:
    ...
    def test_enter_multiple_caches(self):
        ...
        with self.cache:
            with self.assertRaisesRegex(
                ValueError, "cannot enter multiple caches.*" f"{self.cache.path}"
            ):
                with cache:
                    pass
    ...
    def test_enter_multiple_pools(self):
        ...
        with pool1:
            with self.assertRaisesRegex(
                ValueError, "cannot enter multiple pools.*" f"{pool1.path}"
            ):
                with pool2:
                    pass
    ...
    def test_loading_pool(self):
        ...
        with self.assertRaisesRegex(ValueError, "'pool' does not point to any data"):
    ...
    def test_dangling_reference_in_pool(self):
        ...
        shutil.rmtree(self.cache.data / str(ref.uuid))
        with self.assertWarnsRegex(Warning, f"Dangling reference .*{ref.uuid}"):
            self.cache.garbage_collection()
    ...
    @pytest.mark.skipif(os.geteuid() != 0, reason="only sudo can mess with users")
    @pytest.mark.skipif(
        platform.system() == "Darwin", reason="Mac clusters not really a thing"
    )
    def test_multi_user(self):
        ...
        # things in it
        with root_cache:
            root_result = concatenate_ints(self.art1, self.art2, self.art4, 4, 5)[0]
        ...
        with _fake_user_for_cache(
            cache_prefix, i_acknowledge_this_is_dangerous=True
        ) as (uname, user_cache):
            with user_cache:
            ...
            user_expected = set(
                (
                    "./VERSION",
                    f"data/{user_result._archiver.uuid}",
                )
            )
