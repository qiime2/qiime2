...
from qiime.interface.q2d3 import get_plugin_workflow
        ...
        def post(self, plugin, workflow):
            ...
            for argument in self.request.arguments:
                ...
                if argument.startswith('in-'):
                    ...
                    if not value:
                        raise TypeError(
                            "Must enter an output name for the artifact.")
...
#
#     def prepare(self):
#         self._boundary = b'--' + ''.join(
#             self.request.headers.get('Content-Type').split('=')[1:]).encode(
#                 'ascii')
...
def start_server(port=4444):
    ...
    process = subprocess.Popen(
        "jupyter notebook --no-browser --port %d --port-retries 0" % jport,
        shell=True)

    @atexit.register
    def shutdown():
        subprocess.call(
            "ps -ef | awk '$3 == \"%d\" {print $2}' | xargs kill -15"
            % process.pid, shell=True)
        process.kill()
