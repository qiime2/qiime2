...
class FieldSignature(namedtuple('FieldSigTuple', ['variant_types'])):
    # Don't uncomment me, I will recurse forever
...
class BaseType(metaclass=TypeMeta, fields='Primitive'):
...
class _SimpleNotUnion(BaseType, generic=True):
...
class _NotType(BaseType, generic=True):
    def __init__(self, type_):
        super().__init__()
        self._instance = type_()
...
# TODO create a base class for Any, Nil, and BaseType
class Any:
    def __new__(cls):
