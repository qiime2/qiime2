...
class IdentityTests:
    ...
    def test_subtype_identities(self):
        ...
        # opposite
        self.assertExclusiveDisjunction(
            self.Foo[self.Bar], ~self.Foo[self.Bar])
...
class TestUnvariantLeafAlgebraicDataTypes(unittest.TestCase, IdentityTests):
    def setUp(self):
        class Foo(BaseType, fields='X'):
            class X:
                pass

        class Bar(BaseType, variant_of=Foo.X):
            pass

        class Baz(BaseType, variant_of=Foo.X):
            pass
...
class TestUnvariantVariantLeafAlgebraicDataTypes(unittest.TestCase,
                                                 IdentityTests):
    def setUp(self):
        class Foo(BaseType, fields='A'):
            class A:
                pass

        class Bar(BaseType, fields='B', variant_of=Foo.A):
            class B:
                pass

        class Baz(BaseType, variant_of=Bar.B):
            pass
    ...
    def test_deep_nest(self):
        ...
        self.assertTrue(
            self.Foo[self.Bar[self.Baz]] < ~self.Foo[self.Bar[~self.Baz]])
        self.assertFalse(
            self.Foo[self.Bar[self.Baz]] > ~self.Foo[self.Bar[~self.Baz]])
...
class TestVariantVariant(unittest.TestCase, IdentityTests):
        def setUp(self):
            class _(BaseType, fields='A'):
                class A:
                    pass

            class Foo(BaseType, fields='B', variant_of=_.A):
                class B:
                    pass

            class Bar(BaseType, variant_of=Foo.B):
                pass
